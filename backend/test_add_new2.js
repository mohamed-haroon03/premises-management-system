...
async function run() {
    ...
    try {
        ...
        log.push("Post res: " + JSON.stringify(postRes.data));
        ...
        log.push("Pending Rent: " + res.data.pendingRent);
        log.push("Monthly Income: " + res.data.monthlyIncome);
    } catch (err) {
        if (err.response) {
    ...
    fs.writeFileSync('test_add_new_out.txt', log.join('\n'));
}
run();
